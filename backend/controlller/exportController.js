...
const processInlineContent = (children) => {
  const textRuns = [];
  const currentFormatting = { bold: false, italic: false };
  let textBuffer = "";
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        console.error("Cloudinary image embed failed:", imgErr.message);
      }
    }
    ...
    if (book.chapters?.length) {
      book.chapters.forEach((chapter, index) => {
        try {
          ...
          console.error(`Chapter ${index + 1} failed:`, chapterError);
        }
      });
    }
